...
procedure Transforme_Image is
   ...
   type T_Entete is record 
	...
	-- Lie l'entente du fichier image
   ...
   begin
	...
	-- lie l'image et la stock dans l'attribut Image : T_Image
   ...
   monImage : T_Image(1..monEntete.Dim_X,1..monEntete.Dim_Y);
	...
	-- Transforme l'image en négatif
	procedure Negatif(Image : in out T_Image) is
	begin
	...
	-- Applique une symetrie Vertical à l'image
	procedure Symetrie_Verticale(Image : in out T_Image) is
		tmpImage : T_Image := Image;
	begin
		for I in monImage'Range(1) loop
		   for J in monImage'Range(2) loop
		      monImage(I,J) := tmpImage(I, monImage'Length(2) - J + 1);
		   end loop;
	   end loop;
	end Symetrie_Verticale;

		-- Applique une horizontale Vertical à l'image
	procedure Symetrie_Horizontale(Image : in out T_Image) is
		tmpImage : T_Image := Image;
	begin
		for I in monImage'Range(1) loop
		   for J in monImage'Range(2) loop
		      monImage(I,J) := tmpImage(monImage'Length(2) - I + 1, J);
		   end loop;
	   end loop;
	end Symetrie_Horizontale;

	procedure Photomaton(Image : T_Image) is
	begin
		--for I in monImage'Range(1) loop
		--for J in monImage'Range(2) loop		
		--end loop;
...
begin
   lire(monImage,"lena.pgm");
	...
	--Negatif(monImage);
	Symetrie_Horizontale(monImage);
	Symetrie_Verticale(monImage);
	Photomaton(monIMage);
   

   ecrire(monEntete,monImage,"sortie.pgm");

end Transforme_Image;
